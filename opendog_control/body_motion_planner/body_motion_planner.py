import numpy as np
from cmd_manager.opendog_variables import Cmds, Body, Leg
import time
# cmd = Cmds()
# leg = Leg()
# body = Body()
...
class BodyMotionPlanner():
    def __init__(self , cmd, leg, body, gait_planner):
        ...
        # Height ramp state: slews foot_zero_pnt height toward cmd.body.height
        # instead of snapping instantly, to avoid current-limit spikes on
        # hardware when standing up from rest.
        self._ramped_height = float(self.cmd.body.height)
        self._last_ramp_time = time.time()
        self.HEIGHT_RAMP_RATE = 60.0  # mm/second
    ...
    def run(self):
        while self.running:
            if self.cmd.mode.start:
                ...
                target_height = float(self.cmd.body.height)
                max_step = self.HEIGHT_RAMP_RATE * dt
                delta = target_height - self._ramped_height
                if delta > max_step:
                    delta = max_step
                elif delta < -max_step:
                    delta = -max_step
                self._ramped_height += delta
                self.cmd.leg.foot_zero_pnt[:,2] = np.array(self._ramped_height)

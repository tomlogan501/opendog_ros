"""
Full FL leg hardware test pipeline: real CAN hardware interface +
...
def generate_launch_description():
    ...
    hardware_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_hw, 'launch', 'fl_leg_test.launch.py')
        )
    )
    ...
    joint_ctrl_bridge_node = Node(package='opendog_hardware_layer_can',
                                  executable='fl_leg_test_joint_ctrl_node',
                                  name='fl_leg_test_joint_ctrl_node', output='screen')

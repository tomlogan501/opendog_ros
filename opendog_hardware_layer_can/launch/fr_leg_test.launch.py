...
def generate_launch_description():
    ...
    robot_controllers = PathJoinSubstitution([
        ...
        "config", "fr_leg_test_controllers.yaml",
    ])
    ...
    fr_spawner = Node(package="controller_manager", executable="spawner",
                      arguments=["fr_leg_test_controller", "-c", "/controller_manager"])

    return LaunchDescription(declared_arguments + [
        control_node,
        robot_state_pub_node,
        TimerAction(period=3.0, actions=[jsb_spawner]),
        TimerAction(period=5.0, actions=[fr_spawner]),
    ])

...
def generate_launch_description():
    ...
    robot_controllers = PathJoinSubstitution([
        ...
        "config", "back_test_controllers.yaml",
    ])
    ...
    back_spawner = Node(package="controller_manager", executable="spawner",
                        arguments=["back_test_controller", "-c", "/controller_manager"])

    return LaunchDescription(declared_arguments + [
        control_node,
        robot_state_pub_node,
        TimerAction(period=3.0, actions=[jsb_spawner]),
        TimerAction(period=5.0, actions=[back_spawner]),
    ])

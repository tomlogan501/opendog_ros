"""
Front-of-robot hardware test: FL + FR legs (6 joints) across ODrive
0, 1 and 3. No back-leg boards required.
"""
...
def generate_launch_description():
    ...
    hardware_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_hw, 'launch', 'front_test.launch.py')
        )
    )
    ...
    joint_ctrl_bridge_node = Node(package='opendog_hardware_layer_can',
                                  executable='front_test_joint_ctrl_node',
                                  name='front_test_joint_ctrl_node', output='screen')

...
//
// For this isolated 2-joint hardware test, only indices 7 (BR_uleg)
// and 8 (BR_lleg) are extracted and republished, in that order, to
// br_leg_test_controller/commands -- matching that controller's
// declared joints: ["BR_uleg_joint", "BR_lleg_joint"].
//
// IMPORTANT: same as the full bridge node, this does NOT clamp
// values to any safe range. Legs must be physically unloaded /
// restrained during testing.
class BrLegTestJointCtrl : public rclcpp::Node
{
public:
    BrLegTestJointCtrl()
        : Node("br_leg_test_joint_ctrl_node")
        {
            subscription_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
                "opendog_jointController/commands", 30,
                std::bind(&BrLegTestJointCtrl::topic_callback, this, _1));

            publisher_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
                "br_leg_test_controller/commands", 30);
        }
...
private:
    ...
    {
        if (msg_rx->data.size() <= BR_LLEG_INDEX) {
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<BrLegTestJointCtrl>());
    rclcpp::shutdown();
    return 0;
}

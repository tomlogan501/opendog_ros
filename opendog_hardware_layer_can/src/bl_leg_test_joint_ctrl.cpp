...
//
// For this isolated 2-joint hardware test, only indices 10 (BL_uleg)
// and 11 (BL_lleg) are extracted and republished, in that order, to
// bl_leg_test_controller/commands -- matching that controller's
// declared joints: ["BL_uleg_joint", "BL_lleg_joint"].
//
// IMPORTANT: this does NOT clamp values to any safe range. The leg
// must be physically unloaded / restrained during testing.
class BlLegTestJointCtrl : public rclcpp::Node
{
public:
    BlLegTestJointCtrl()
        : Node("bl_leg_test_joint_ctrl_node")
        {
            subscription_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
                "opendog_jointController/commands", 30,
                std::bind(&BlLegTestJointCtrl::topic_callback, this, _1));

            publisher_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
                "bl_leg_test_controller/commands", 30);
        }
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<BlLegTestJointCtrl>());
    rclcpp::shutdown();
    return 0;
}

...
//
// For this isolated 2-joint hardware test, only indices 4 (FL_uleg)
// and 5 (FL_lleg) are extracted and republished, in that order, to
// fl_leg_test_controller/commands.
//
// IMPORTANT: this does NOT clamp values to any safe range. The leg
// must be physically unloaded / restrained during testing.
class FlLegTestJointCtrl : public rclcpp::Node
{
public:
    FlLegTestJointCtrl()
        : Node("fl_leg_test_joint_ctrl_node")
        {
            subscription_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
                "opendog_jointController/commands", 30,
                std::bind(&FlLegTestJointCtrl::topic_callback, this, _1));

            publisher_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
                "fl_leg_test_controller/commands", 30);
        }
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<FlLegTestJointCtrl>());
    rclcpp::shutdown();
    return 0;
}

...
class LegWalkingController(Node):
    ...
    def __init__(self):
        ...
        self.timer = self.create_timer(0.1, self.timer_callback)  # Reduced timer period for smoother motion 0.1
        self.step_sequence = self.generate_vertical_circle_sequence(radius=0.5, num_points=36) # 1 36
        self.current_step = 0
    ...
    def generate_vertical_circle_sequence(self, radius, num_points):
        ...
        for i in range(num_points):
            ...
            x = 0.0  # Assuming movement in a vertical plane, keep x constant
    ...
    def publish_trajectory(self, positions):
        msg = JointTrajectory()
        msg.joint_names = ['ul_shoulder_to_base_link', 'ul_upperleg_to_shoulder', 'ul_lowerleg_to_upperleg'] #'ul_shoulder_to_base_link', 'ul_upperleg_to_shoulder', 'ul_lowerleg_to_upperleg'  # 'joint1','joint2'
        ...
        self.get_logger().info(f"Published joints trajectory command: {positions}")

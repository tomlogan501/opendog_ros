#!/bin/bash
# Script to put all ODrives into CLOSED_LOOP_CONTROL via CAN
# OpenDog v3 - 12 axes (Node IDs 0-11)

echo "=========================================="
echo "STARTING CLOSED_LOOP_CONTROL via CAN"
echo "=========================================="

# Check that CAN0 is UP
if ! ip link show can0 | grep -q "UP"; then
    echo "Error: can0 is not UP"
    echo "Run: sudo ip link set can0 up type can bitrate 250000"
    exit 1
fi

echo "CAN0 interface active"
echo ""

# ODrive command: Set Axis State = 8 (CLOSED_LOOP_CONTROL)
# Format: cansend can0 <CMD_ID>#<DATA>
# CMD_ID = 0x007 + (node_id << 5)
# DATA = 08000000 (little-endian, requested_state = 8)

echo "Sending CLOSED_LOOP_CONTROL commands..."
...
echo "Commands sent."
echo ""
echo "Checking states (heartbeat)..."
echo "Looking for state 8 (CLOSED_LOOP) in messages..."
echo ""

# Capture a few heartbeats
candump can0 -n 30 2>&1 | grep "can0  0[0-9a-f][0-9a-f]   \[8\]" | head -12

echo ""
echo "=========================================="
echo "If you see '08 00 00 00' -> SUCCESS"
echo "If you see '01 00 00 00' -> still IDLE"
echo "=========================================="

...
// ─────────────────────────────────────────────────────────
// on_init
// Called once at startup — reads joint config from URDF
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// on_activate
// Called when robot activates — opens CAN socket,
// clears errors, calibrates and enters closed loop
// ─────────────────────────────────────────────────────────
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Wait for heartbeat from each motor
  for (size_t i = 0; i < n_joints_; i++) {
    uint32_t axis_error;
    uint8_t  axis_state;
...
// ─────────────────────────────────────────────────────────
// on_deactivate
// Called on shutdown — stops all motors and closes socket
// ─────────────────────────────────────────────────────────
...
// Tells ros2_control what data we provide (pos, vel, effort)
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// read — called every 1ms by ros2_control
// Reads position + velocity from ODrive over CAN
  ...
  const rclcpp::Duration & /*period*/)
{
  for (size_t i = 0; i < n_joints_; i++) {
    ...
    if (get_encoder_estimates(can_sock_, node_ids_[i], pos, vel, 10)) {
      // ODrive uses turns — convert to radians for ROS2
      hw_positions_[i]  = static_cast<double>(pos) * 2.0 * M_PI;
      hw_velocities_[i] = static_cast<double>(vel) * 2.0 * M_PI;
    }
  }
...
// ─────────────────────────────────────────────────────────
// write — called every 1ms by ros2_control
// Sends position commands to ODrive over CAN
  ...
  const rclcpp::Duration & /*period*/)
{
  for (size_t i = 0; i < n_joints_; i++) {
    ...
    float pos = static_cast<float>(hw_commands_positions_[i] / (2.0 * M_PI));
    float vel = static_cast<float>(hw_commands_velocities_[i] / (2.0 * M_PI));

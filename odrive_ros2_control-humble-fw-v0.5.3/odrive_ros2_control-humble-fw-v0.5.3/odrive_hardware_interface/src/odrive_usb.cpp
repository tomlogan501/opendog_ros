...
{
  RCLCPP_INFO(rclcpp::get_logger("Test logger"),"odrive_usb.cpp init start");
  
  int ret = libusb_init(&libusb_context_);
  RCLCPP_INFO(rclcpp::get_logger("Test logger"),"start libusb conditional");
  std::cout << "The value of serial_numbers are: " << serial_numbers[0][0] << std::endl;
  std::cout << "The value of serial_numbers are: " << serial_numbers[0][1] << std::endl;
  std::cout << "The value of serial_numbers are: " << serial_numbers[0][2] << std::endl;
  if (ret != LIBUSB_SUCCESS) {
    RCLCPP_INFO(rclcpp::get_logger("Test logger"),"odrive_usb.cpp LIBUSB SUCCESS");
    return ret;
  }
  RCLCPP_INFO(rclcpp::get_logger("Test logger"),"start second conditional");
  libusb_device ** device_list;
  ssize_t device_count = libusb_get_device_list(libusb_context_, &device_list);
  if (device_count <= 0) {
    RCLCPP_INFO(rclcpp::get_logger("Test logger"),"device count = 0");
    return device_count;
  }
  RCLCPP_INFO(rclcpp::get_logger("Test logger"),"start for loop");
  for (ssize_t i = 0; i < device_count; ++i) {
    libusb_device * device = device_list[i];
    libusb_device_descriptor descriptor;
    RCLCPP_INFO(rclcpp::get_logger("Test logger"),"start if lines");
    if (libusb_get_device_descriptor(device, &descriptor) != LIBUSB_SUCCESS) {
      RCLCPP_INFO(rclcpp::get_logger("Test logger"),"USB not detected");
      continue;
    }
    RCLCPP_INFO(rclcpp::get_logger("Test logger"),"start if lines 2");
    if (
      ...
      if (libusb_open(device, &device_handle) != LIBUSB_SUCCESS) {
        RCLCPP_INFO(rclcpp::get_logger("Test logger"),"first if");
        continue;
      }
      if (
        ...
        RCLCPP_INFO(rclcpp::get_logger("Test logger"),"second if");
        continue;
      }
      if ((libusb_claim_interface(device_handle, 2)) != LIBUSB_SUCCESS) {
        RCLCPP_INFO(rclcpp::get_logger("Test logger"),"third if");
        libusb_close(device_handle);
        continue;
      }
      ...
      if ((read(device_handle, SERIAL_NUMBER, serial_number)) != LIBUSB_SUCCESS) {
        ...
        libusb_close(device_handle);
        RCLCPP_INFO(rclcpp::get_logger("Test logger"),"4th if");
        continue;
      }
      odrive_map_.insert(std::pair<int64_t, libusb_device_handle *>(-serial_number, device_handle));
    }
  }
  RCLCPP_INFO(rclcpp::get_logger("Test logger"),"end of if lines 2");
  libusb_free_device_list(device_list, 1);
  if (!odrive_map_.size()) {
    RCLCPP_INFO(rclcpp::get_logger("Test logger"),"No device detecter");
